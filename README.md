...
<dl>
    <dt>メール認証について</dt>
    <dd>メール認証は「mailtrap」を使用しました。ユーザー登録をしていない場合は公式サイトより登録をお願いします。</dd>
</dl>

<dl>
    <dt>勤怠情報の詳細ページについて</dt>
    <dd>出勤のみ打刻してまだ退勤の打刻をしていない場合や、勤怠記録の無い日は「詳細」ボタンを表示しないようにしました。これは勤怠がまだ確定していないのに、修正依頼をかけるのを防ぐ目的です。</dd>
</dl>

<dl>
    <dt>勤怠情報の修正機能について</dt>
    <dd>出勤時間、退勤時間、休憩時間の時刻を入力するときは、例えば「1時5分」のように時や分の桁数が1桁のときでも入力の際には、「01:05」のような書式で入力をして下さい。0埋めのない入力はエラーと見なします。出勤時間と退勤時間、休憩入時間と休憩戻時間の入力値は互いが同時刻の場合はエラーと見なします。</dd>
</dl>

<dl>
    <dt>バリデーションファイル(Requestファイル)について</dt>
    <dd>管理者と一般ユーザーで、ログイン画面のフォーム及び、勤怠修正のフォームは同じ入力内容のため、バリデーションファイルは同じものを使用しています。</dd>
</dl>
